...
const CANVAS_COLOR = [0.0, 0.9, 0.5, 1.0]; //color for the background of the canvas
...
function draw() {
    ...
    // set the color of the background to yellow
    ...
    let cubeAngle = DEFAULT_ANGLE;
    let orbitRadius = 3; 
    let cubeCenter = [
        orbitRadius * Math.cos(offset * 0.02), 
        1,
        orbitRadius * Math.sin(offset * 0.02), 
    ];
    let cubeRotationAxis = [5, 0, 0];
    ...
    // cone
    let coneData = uvCone(0.5, 0.5, 20);
    let cone = new ShapeBuffer(coneData, cubeAngle, cubeRotationAxis, [0, 1 + Math.cos(offset * 0.01), -9]);

    // cylinder
    let cylinderData = uvCylinder(0.5, 0.2, 20);
    let cylinderAngle = DEFAULT_ANGLE + offset;
    let cylinderRotationAxis = [-5, 3, 0];
    let cylinderCenter = cylinderRotationAxis
    let cylinder = new ShapeBuffer(cylinderData, cylinderAngle, cylinderRotationAxis, cylinderCenter);
    
    // sphere
    let sphereData = uvSphere(0.5, 20, 20);
    let sphereCenter = [0, 5 + Math.cos(offset * 0.25), -2 + Math.sin(offset * 0.25)];
    let sphere = new ShapeBuffer(sphereData, DEFAULT_ANGLE, Y_AXIS, sphereCenter);
